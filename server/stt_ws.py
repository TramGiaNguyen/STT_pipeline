...
class STTWebSocketHandler:
    ...
    def __init__(
        ...
        self.min_audio_length: float = 0.25  # Giảm xuống 0.25s (chuẩn Silero min speech)
        self.max_buffer_duration: float = 10.0  # Tối đa 10s buffer
    ...
    async def _transcribe_and_respond(self, audio: np.ndarray):
        ...
        try:
            # Chạy STT với các tham số chống hallucination
            results = await run_in_threadpool(
                ...
                no_speech_threshold=0.5,           # SIẾT: 0.6 → 0.5 (reject silence sớm hơn)
                compression_ratio_threshold=2.2,   # Siết chặn hallucination loop từ 2.4 -> 2.2
            )
            ...
            if results:
                ...
                if len(words) > 5:  # Chỉ check nếu có > 5 từ
                    ...
                    if repetition_ratio < 0.4:
                        logger.warning(f"Hallucination: lặp lại {repetition_ratio:.1%}, bỏ qua. Text: '{full_text}'")
                        await self._send_result("")
                        return
    ...
    async def _handle_audio(self, message: dict):
        ...
        # Bước 4: VAD confirm — bây giờ mới normalize để Whisper nhận audio chuẩn.
        # Normalize chỉ áp dụng cho audio hợp lệ (đã biết là có tiếng nói).
        target_rms = 0.1
        audio_normalized = audio_raw * (target_rms / rms_raw)

        # Thêm audio đã normalize vào buffer
    ...
    async def _flush_buffer(self, force: bool = False):
        ...
        # Chỉ transcribe nếu đủ dài VÀ đủ năng lượng
        if buffered_duration >= self.min_audio_length and buffer_rms >= MIN_AUDIO_ENERGY:
            logger.info(
            ...
            logger.info(
                ...
                f"RMS={buffer_rms:.6f} (min_dur={self.min_audio_length}, "
